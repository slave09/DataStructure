...
private:
	int size;
	int doubleKey ; // First prime number from last of the array
	int *Array;
...
int HashMap :: second_hash(int val){
	return doubleKey - (val % doubleKey); // gives random index
}
/*
	* Double hashing = h'(x) = (firstHasing + f(i)*secondHashing) % 10
	* f(i) = i
	* i = 0,1,2,3....
*/
